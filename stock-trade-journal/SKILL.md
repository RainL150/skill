...
- 必须给“可选值 + 自由补充”的问法，帮助用户快速录入。例如主逻辑可提示：估值便宜、增长超预期、困境反转、周期底部、情绪错杀、事件催化；周期可提示：短线 / 中线 / 长线；失效条件可提示：财报转弱、核心指标低于 X、监管/政策恶化、价格跌破且基本面变坏；加仓规则可提示：只在新财报/经营数据增强、回踩企稳、估值更便宜且失效条件未触发时加。
- 用户可以对任一笔说“跳过”“先不补”“以后再补”。跳过时不要编造 note，也不要阻塞其他导入；最终汇总列出跳过的交易。
- 若用户回答不完整，允许先写入已回答字段，并标注缺少哪些 profile 必填字段。
- 若用户提供一句自然语言总结，需整理成 profile 字段格式后再写入 `trades.note`，并同步写入 `notes` 的 `trade_decision`，关联对应交易。

写入格式仍使用单条 `note` 字段：

```text
主逻辑: <答案>；周期: <短线/中线/长线>；失效条件: <可观察条件>；加仓规则: <可加/不可加条件>；仓位上限: <可选>；目标价/观察价: <可选>；复盘日期: <可选>
```
