...
def update_position_after_trade(conn: sqlite3.Connection, trade: dict):
    """
    交易后更新持仓表
    ...
    if row is None:
        ...
        if side == "BUY":
            conn.execute("""
                INSERT INTO positions (
                    ts_code, quantity, avg_cost, total_cost, currency,
                    first_buy_date, last_trade_date, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                ts_code, quantity, price, price * quantity, currency,
            ...
            """, (
                ts_code, -quantity, price, 0, currency,
                timestamp, timestamp, datetime.now().isoformat()
            ))
    else:
        # 更新持仓
        ...
        """, (
            ...
            timestamp, datetime.now().isoformat(), ts_code
        ))

    conn.commit()


def sync_positions_from_ibkr(conn: sqlite3.Connection, ibkr_positions: list):
    """
    从 IBKR 同步持仓数据
    ...
    for pos in ibkr_positions:
        ...
        conn.execute("""
            INSERT INTO positions (
                ts_code, quantity, avg_cost, total_cost, market_price,
        ...
        """, (
            ts_code, quantity, avg_cost, avg_cost * quantity if avg_cost else 0,
...
def get_positions(conn: sqlite3.Connection, include_zero: bool = False) -> list:
    """获取所有持仓"""
    if include_zero:
...
def get_position(conn: sqlite3.Connection, ts_code: str) -> Optional[dict]:
    """获取单个股票持仓"""
...
def take_position_snapshot(conn: sqlite3.Connection, snapshot_date: str = None):
    """保存持仓快照"""
